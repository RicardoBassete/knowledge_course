  ...
  <div class="article-admin">
    <b-form>
      ...
      <b-form-group label="Nome:" label-for="article-name">
        <b-form-input id="article-name" type="text" v-model="article.name"
          :readonly="mode === 'remove'" required placeholder="Informe o nome do artigo..."
        />
      </b-form-group>
      <!-- Segunda linha - descrição -->
      <b-form-group label="Descrição:" label-for="article-description">
        <b-form-input id="article-description" type="text" v-model="article.description"
          :readonly="mode === 'remove'" required placeholder="Informe a descrição do artigo..."
        />
      </b-form-group>
      <!-- Terceira linha - url da imagem -->
      <b-form-group label="Imagem (URL):" label-for="article-imageUrl" v-if="mode === 'save'">
        <b-form-input id="article-imageUrl" type="text" v-model="article.imageUrl"
          :readonly="mode === 'remove'" required placeholder="Informe a URL da imagem do artigo..."
        />
      </b-form-group>
      <!-- Quarta linha - categoria -->
    ...
    <b-table hover striped :items="articles" :fields="fields">
      <template slot="cell(actions)" slot-scope="data">
        <!-- botao de editar categoria -->
        <b-button variant="warning" @click="loadArticle(data.item)" class="mr-2">
          <i class="fa fa-pencil"></i>
        </b-button>
        <!-- botao de excluir categoria -->
